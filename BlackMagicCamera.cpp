...
bool BlackMagicCamera::setup(int device, int videomode,InputModes conversionMode){
	...
	{
		cout << "This application requires the Desktop Video drivers installed. Please install the Blackmagic Desktop Video drivers to use the features of this application." << endl;
		goto bail;
	}
...
vector<string> BlackMagicCamera::getInputList(){
    ...
    nameList.clear();
    cout << "Blackmagic Devices found" << endl;
...
bool BlackMagicCamera::selectDevice(int deviceID, int videoModeIndex, InputModes inputMode )
{
	...
	{
		cout << "This application was unable to select the device. Error getting selecting the DeckLink device." << endl;
		goto bail;
	}
	...
	{
		cout << "This application was unable to obtain IDeckLinkInput for the selected device. Error getting setting up capture." << endl;
		deckLinkInput = NULL;
		goto bail;
	}
	...
	{
		cout << "An invalid display mode was selected. Error starting the capture" << endl;
		return false;
	}
	...
	{
        if (hr==E_INVALIDARG) cout << "E_INVALIDARG" << endl;
        if (hr==E_ACCESSDENIED) cout << "E_ACCESSDENIED" << endl;
        if (hr==E_OUTOFMEMORY) cout << "E_OUTOFMEMORY" << endl;
        if (hr==E_FAIL) cout << "E_FAIL" << endl;
		cout << "This application was unable to select the chosen video mode. Perhaps, the selected device is currently in-use. Error starting the capture " << hr << endl;
		return false;
	}
...
bool BlackMagicCamera::startCapture()
{
	...
	{
		cout << "This application was unable to start the capture. Perhaps, the selected device is currently in-use. Error starting the capture" << endl;
		return false;
	}
...
//	NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
	...
	// Restart capture with the new video mode if told to
		...
		{
			stopCapture();
			cout << "This application was unable to select the new video mode. Error restarting the capture." << endl;
			goto bail;
		}
		...
		{
			stopCapture();
			cout << "This application was unable to start the capture on the selected device. Error restarting the capture." << endl;
...
{
    //cout << " newFrame " << mNewFrameAvailable << endl;
	...
	*/
    ...
    if (mInputMode==INPUTMODE_10YUV || mInputMode==INPUTMODE_10YUVTORGB || mInputMode==INPUTMODE_10RGB){
        //create a surface
       mSurface32.allocate(videoFrame->GetWidth(),videoFrame->GetHeight(),OF_IMAGE_COLOR_ALPHA);
    }else{
        //create a surface
        mSurface.allocate(videoFrame->GetWidth(),videoFrame->GetHeight(),OF_IMAGE_COLOR);
    }
    ...
    if (mInputMode==INPUTMODE_10YUV || mInputMode==INPUTMODE_10YUVTORGB || mInputMode==INPUTMODE_10RGB){
        ...
        //if (mSurface32Buffer.size()>1) cout << mSurface32Buffer.size() << " frames available" << endl;
    }else{
        ...
        //if (mSurfaceBuffer.size()>1) cout << mSurfaceBuffer.size() << " frames available" << endl;
    }
